...
void PlaneSurface::gen(cv::Mat_<cv::Vec3f> *coords, cv::Mat_<cv::Vec3f> *normals, cv::Size size, const cv::Vec3f &ptr,
                       float scale, const cv::Vec3f &offset) {
    bool create_normals = (normals != nullptr) || (offset[2] != 0.0f) || (ptr[2] != 0.0f);
...
QuadSurface::QuadSurface(const cv::Mat_<cv::Vec3f> &points, const cv::Vec2f &scale) : QuadSurface(
    new cv::Mat_<cv::Vec3f>(points.clone()), scale) {
}
...
void QuadSurface::gen(cv::Mat_<cv::Vec3f> *coords, cv::Mat_<cv::Vec3f> *normals, cv::Size size, const cv::Vec3f &ptr,
                      float scale, const cv::Vec3f &offset) {
    bool create_normals = (normals != nullptr) || (offset[2] != 0.0f) || (ptr[2] != 0.0f);
    ...
    std::vector<cv::Vec2f> src = {
        off2d, off2d + cv::Vec2f((w + 8) * _scale[0] / scale, 0), off2d + cv::Vec2f(0, (h + 8) * _scale[1] / scale)
    };
...
//plane -> stay on plane
float min_loc(const cv::Mat_<cv::Vec3f> &points, cv::Vec2f &loc, cv::Vec3f &out, const std::vector<cv::Vec3f> &tgts,
              const std::vector<float> &tds, const PlaneSurface *plane, float init_step, float min_step) {
...
template<typename E>
static float search_min_loc(const cv::Mat_<E> &points, cv::Vec2f &loc, cv::Vec3f &out, const cv::Vec3f& tgt,
                            const cv::Vec2f& init_step, float min_step_x) {
...
template<typename E>
static float _pointTo(cv::Vec2f &loc, const cv::Mat_<E> &points, const cv::Vec3f &tgt, float th, int max_iters, float scale) {
...
float pointTo(cv::Vec2f &loc, const cv::Mat_<cv::Vec3d> &points, const cv::Vec3f &tgt, float th, int max_iters,
              float scale) {
    return _pointTo(loc, points, tgt, th, max_iters, scale);
}

float pointTo(cv::Vec2f &loc, const cv::Mat_<cv::Vec3f> &points, const cv::Vec3f &tgt, float th, int max_iters,
              float scale) {
    return _pointTo(loc, points, tgt, th, max_iters, scale);
}
...
QuadSurface *load_quad_from_obj(const std::string &path) {
    ...
    for (const auto &uv: texcoords | std::views::values) {
    ...
    auto *points = new cv::Mat_<cv::Vec3f>(height, width, cv::Vec3f(-1, -1, -1));
    ...
    for (const auto &face: faces) {
        for (size_t i = 0; i < face.v_indices.size() && i < face.vt_indices.size(); i++) {
            ...
            if (vertices.contains(v_idx) && texcoords.contains(vt_idx)) {
...
void ControlPointSurface::addControlPoint(const cv::Vec3f &base_ptr, const cv::Vec3f& control_point) {
    _controls.emplace_back(this, base_ptr, control_point);
}

void ControlPointSurface::gen(cv::Mat_<cv::Vec3f> *coords_, cv::Mat_<cv::Vec3f> *normals_, cv::Size size,
                              const cv::Vec3f &ptr, float scale, const cv::Vec3f &offset) {
...
void RefineCompSurface::gen(cv::Mat_<cv::Vec3f> *coords_, cv::Mat_<cv::Vec3f> *normals_, cv::Size size,
                            const cv::Vec3f &ptr, float scale, const cv::Vec3f &offset) {
    cv::Mat_<cv::Vec3f> _coords_local;
...
static void set_block(cv::Mat_<uint8_t> &block, const cv::Vec3f &last_loc, const cv::Vec3f &loc, const cv::Rect &roi,
               float step) {
...
void find_intersect_segments(std::vector<std::vector<cv::Vec3f> > &seg_vol,
                             std::vector<std::vector<cv::Vec2f> > &seg_grid, const cv::Mat_<cv::Vec3f> &points,
                             PlaneSurface *plane, const cv::Rect &plane_roi, float step, int min_tries) {
    ...
    for (int r = 0; r < std::max(min_tries, std::max(points.cols, points.rows) / 100); r++) {
        ...
        for (int i = 0; i < std::max(min_tries, std::max(points.cols, points.rows) / 100); i++) {
            ...
            if (get_block(block, plane_loc, plane_roi, block_step) != 0u)
                dist = -1;
        ...
        std::ranges::reverse(seg2);
        std::ranges::reverse(seg_loc2);
    ...
    if (_bbox.low[0] == -1) {
        ...
        for (int j = 0; j < _points->rows; j++)
            for (int i = 0; i < _points->cols; i++)
                if (_bbox.low[0] == -1)
                    _bbox = {.low=(*_points)(j, i), .high=(*_points)(j, i)};
...
QuadSurface *load_quad_from_tifxyz(const std::string &path) {
    std::vector<cv::Mat_<float> > xyz = {
        cv::imread(path + "/x.tif", cv::IMREAD_UNCHANGED), cv::imread(path + "/y.tif", cv::IMREAD_UNCHANGED),
        cv::imread(path + "/z.tif", cv::IMREAD_UNCHANGED)
    };
    ...
    auto *surf = new QuadSurface(points, scale);
...
Rect3D rect_from_json(const nlohmann::json &json) {
    return {.low={json[0][0], json[0][1], json[0][2]}, .high={json[1][0], json[1][1], json[1][2]}};
}
...
bool contains(SurfaceMeta &a, const cv::Vec3f &loc, int max_iters) {
    if (!intersect(a.bbox, {.low=loc, .high=loc}))
        return false;

    cv::Vec3f ptr = a.surface()->pointer();
    return a.surface()->pointTo(ptr, loc, 2.0, max_iters) <= 2.0;
}
...
SurfaceMeta::SurfaceMeta(std::filesystem::path path_, const nlohmann::json &json) : path(std::move(path_)) {
