...
QuadSurface *space_tracing_quad_phys(z5::Dataset *ds, float scale, ChunkCache *cache, cv::Vec3f origin,
                                     int generations = 100, float step = 10, const std::string &cache_root = "",
                                     float voxelsize = 1.0);

QuadSurface *regularized_local_quad(QuadSurface *src, const cv::Vec3f &ptr, int w, int h, int step_search = 100,
                                    int step_out = 5);
...
public:
    ...
    virtual void gen(cv::Mat_<cv::Vec3f> *coords, cv::Mat_<cv::Vec3f> *normals, cv::Size size, const cv::Vec3f &ptr,
                     float scale, const cv::Vec3f &offset) = 0;
...
public:
    ...
    void gen(cv::Mat_<cv::Vec3f> *coords, cv::Mat_<cv::Vec3f> *normals, cv::Size size, const cv::Vec3f &ptr,
             float scale, const cv::Vec3f &offset) override;
    ...
    [[nodiscard]] float scalarp(const cv::Vec3f& point) const;
...
public:
    ...
    friend QuadSurface *regularized_local_quad(QuadSurface *src, const cv::Vec3f &ptr, int w, int h, int step_search,
                                               int step_out);
...
protected:
    ...
    cv::Rect _bounds;
    cv::Vec3f _center;
    Rect3D _bbox = {.low={-1, -1, -1}, .high={-1, -1, -1}};
};
...
public:
    ...
    explicit DeltaSurface(Surface *base);
    ...
    void gen(cv::Mat_<cv::Vec3f> *coords, cv::Mat_<cv::Vec3f> *normals, cv::Size size, const cv::Vec3f &ptr,
             float scale, const cv::Vec3f &offset) override = 0;
...
public:
    ...
    //actual control point location - should be in line with _orig_wp along the normal, but could change if the underlaying surface changes!
};

class ControlPointSurface : public DeltaSurface {
public:
    explicit ControlPointSurface(Surface *base) : DeltaSurface(base) {
    };
...
public:
    ...
    SurfaceMeta(std::filesystem::path path_, const nlohmann::json &json);
...
void find_intersect_segments(std::vector<std::vector<cv::Vec3f> > &seg_vol,
                             ...
                             PlaneSurface *plane, const cv::Rect &plane_roi, float step, int min_tries = 10);

float min_loc(const cv::Mat_<cv::Vec3f> &points, cv::Vec2f &loc, cv::Vec3f &out, const std::vector<cv::Vec3f> &tgts,
              const std::vector<float> &tds, const PlaneSurface *plane, float init_step = 16.0, float min_step = 0.125);

QuadSurface *grow_surf_from_surfs(SurfaceMeta *seed, const std::vector<SurfaceMeta *> &surfs_v,
                                  const nlohmann::json &params, float voxelsize = 1.0);
