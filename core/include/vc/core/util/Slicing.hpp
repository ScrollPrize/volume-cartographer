...
class ChunkCache {
public:
    explicit ChunkCache(size_t size) : _size(size) {
    };
    ...
    bool has(const cv::Vec4i &idx) const;
...
void readInterpolated3D(cv::Mat_<uint8_t> &out, z5::Dataset *ds, const cv::Mat_<cv::Vec3f> &coords,
                        ChunkCache *cache = nullptr);

void readInterpolated2D(cv::Mat_<uint8_t> &out, const z5::Dataset *ds, const cv::Mat_<cv::Vec3f> &coords, ChunkCache *cache);

void readArea3D(xt::xtensor<uint8_t, 3, xt::layout_type::column_major> &out, const cv::Vec3i &offset, const z5::Dataset *ds,
                ChunkCache *cache);
