...
/**
  ...
  A vc::Dictionary defines a set of metadata keys and the associated types
  of the corresponding values (e.g. the key "version" should be interpreted as
  an `int`). A vc::Library holds several such dictionaries: one dictionary
  for each unique version of VolumePkg.

  vc::VERSION_LIBRARY holds the metadata keys and expected value-types for
