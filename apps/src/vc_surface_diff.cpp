...
QuadSurface* load_surface(const fs::path& path) {
    ...
    if (format == "tifxyz") {
        ...
        return load_quad_from_vcps(path.string());
    } else if (format == "obj") {
        return load_quad_from_obj(path.string());
    } else {
        // Try to auto-detect format
        if (fs::exists(path / "x.tif") && fs::exists(path / "y.tif") && fs::exists(path / "z.tif")) {
            return load_quad_from_tifxyz(path.string());
        }
...
int main(int argc, char *argv[])
{
    ...
    try {
        ...
        (*result->meta)["operation"] = operation;
        (*result->meta)["tolerance"] = tolerance;
        (*result->meta)["surface_a"] = surface_a_path.string();
        (*result->meta)["surface_b"] = surface_b_path.string();
        (*result->meta)["timestamp"] = get_timestamp();

        // Add surface A metadata if available
        if (surf_a->meta) {
            (*result->meta)["surface_a_uuid"] = surf_a->meta->value("uuid", "unknown");
        }

        // Add surface B metadata if available
        if (surf_b->meta) {
            (*result->meta)["surface_b_uuid"] = surf_b->meta->value("uuid", "unknown");
        }

        // Add custom parameters if provided
        if (!params.empty()) {
            (*result->meta)["custom_params"] = params;
        }
