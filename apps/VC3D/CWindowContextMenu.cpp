...
void CWindow::onRenderSegment(const SurfaceID& segmentId)
{
    ...
    if (volumePath.isEmpty()) {
        QMessageBox::warning(this, tr("Error"), tr("Cannot render segment: No volume path available"));
        return;
    }
...
void CWindow::onAddOverlap(const SurfaceID& segmentId)
{
    if (_vol_qsurfs.count(segmentId)) {
        QMessageBox::warning(this, tr("Error"), tr("Cannot add overlap: invalid segment selected"));
        return;
    }
...
void CWindow::onGrowSeeds(const SurfaceID& segmentId, bool isExpand, bool isRandomSeed)
{
    ...
    statusBar->showMessage(tr("Seeding volume using %1 in %2").arg(jsonFileName).arg(modeDesc), 5000);
}
